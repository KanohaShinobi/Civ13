...
var/const/JP_BASE_FREQ = 1009
var/const/JP_COMM_FREQ = 1010
var/const/JP_SUPPLY_FREQ = 1011

var/const/US_BASE_FREQ = 1012
var/const/US_COMM_FREQ = 1013
var/const/US_SUPPLY_FREQ = 1014
...
/proc/radio_freq2name(constant)
	...
	switch (constant)
		...
		if (DE_BASE_FREQ)
			return "Wehrmacht Base"
		if (DE_COMM_FREQ)
			return "Wehrmacht Command"
		if (DE_SUPPLY_FREQ)
			return "Wehrmacht Supply"
		if (SS_FREQ)
			return "SS"
		if (UK_FREQ)
			return "Partisans"
		if (JP_BASE_FREQ)
			return "Japanese Base"
		if (JP_COMM_FREQ)
			return "Japanese Command"
		if (JP_SUPPLY_FREQ)
			return "Japanese Supply"
		if (US_BASE_FREQ)
			return "American Base"
		if (US_COMM_FREQ)
			return "American Command"
		if (US_SUPPLY_FREQ)
			return "American Supply"
...
var/global/list/default_japan_channels = list(
	num2text(JP_BASE_FREQ),
	num2text(JP_SUPPLY_FREQ)
)

var/global/list/command_japan_channels = list(
	num2text(JP_BASE_FREQ),
	num2text(JP_SUPPLY_FREQ),
	num2text(JP_COMM_FREQ)
)
var/global/list/default_american_channels = list(
	num2text(US_BASE_FREQ),
	num2text(US_SUPPLY_FREQ)
)

var/global/list/command_american_channels = list(
	num2text(US_BASE_FREQ),
	num2text(US_SUPPLY_FREQ),
	num2text(US_COMM_FREQ)
)
var/global/list/all_channels = default_german_channels | command_german_channels | SS_german_channels | SS_command_german_channels | default_soviet_channels | command_soviet_channels | default_ukrainian_channels | default_japan_channels | command_japan_channels | default_american_channels | command_american_channels
	...
	if (istype(src, /obj/item/radio/intercom))
		...
		if (loc)
			...
			switch (faction)
				...
				if (JAPANESE)
					setup_announcement_system("Arrivals Announcements", (faction == JAPANESE ? JP_BASE_FREQ : US_BASE_FREQ))
					setup_announcement_system("Special Reinforcements Announcements", (faction == JAPANESE ? JP_SUPPLY_FREQ : US_SUPPLY_FREQ))
					setup_announcement_system("Supplydrop Announcements", (faction == JAPANESE ? JP_SUPPLY_FREQ : US_SUPPLY_FREQ))
					setup_announcement_system("Supply Train Announcements", (faction == JAPANESE ? JP_SUPPLY_FREQ : US_SUPPLY_FREQ))
					setup_announcement_system("Reinforcements Announcements", (faction == JAPANESE ? JP_BASE_FREQ : US_BASE_FREQ))
					setup_announcement_system("High Command Announcements", (faction == JAPANESE ? JP_BASE_FREQ : US_BASE_FREQ))
					setup_announcement_system("High Command Private Announcements", (faction == JAPANESE ? JP_COMM_FREQ : US_COMM_FREQ))
					main_radios[JAPANESE] = src
				if (AMERICAN)
					setup_announcement_system("Arrivals Announcements", (faction == AMERICAN ? US_BASE_FREQ : JP_BASE_FREQ))
					setup_announcement_system("Special Reinforcements Announcements", (faction == AMERICAN ? US_SUPPLY_FREQ : JP_SUPPLY_FREQ))
					setup_announcement_system("Supplydrop Announcements", (faction == AMERICAN ? US_SUPPLY_FREQ : JP_SUPPLY_FREQ))
					setup_announcement_system("Supply Train Announcements", (faction == AMERICAN ? US_SUPPLY_FREQ : JP_SUPPLY_FREQ))
					setup_announcement_system("Reinforcements Announcements", (faction == AMERICAN ? US_BASE_FREQ : JP_BASE_FREQ))
					setup_announcement_system("High Command Announcements", (faction == AMERICAN ? US_BASE_FREQ : JP_BASE_FREQ))
					setup_announcement_system("High Command Private Announcements", (faction == AMERICAN ? US_COMM_FREQ : JP_COMM_FREQ))
					main_radios[AMERICAN] = src
...
/obj/item/radio/ui_interact(mob/user, ui_key = "main", var/datum/nanoui/ui = null, var/force_open = TRUE)
	...
	switch (faction)
		...
		if (JAPANESE)
			supply_crate_objects = processes.supply.japan_crate_types.Copy()
			special_reinforcements_requests = processes.supply.japan_special_reinforcements.Copy()
		if (AMERICAN)
			supply_crate_objects = processes.supply.american_crate_types.Copy()
			special_reinforcements_requests = processes.supply.american_special_reinforcements.Copy()
...
/mob/living/carbon/human/proc/post_say(var/message)
	...
	for (var/obj/item/radio/radio in possible_radio_locations)
		...
		if (area)
			...
			switch (faction)
				if (GERMAN)
					if (!istype(area, /area/caribbean/german))
						++radio_delay
				if (RUSSIAN)
					if (!istype(area, /area/caribbean/russian))
						++radio_delay
				if (JAPANESE)
					if (!istype(area, /area/caribbean/japanese))
						++radio_delay
				if (AMERICAN)
					if (!istype(area, /area/caribbean/british))
						++radio_delay

		if (!stuttering || stuttering < 4)
			processes.callproc.queue(radio, /obj/item/radio/proc/broadcast, list((message), src, FALSE), radio_delay)
		else
			processes.callproc.queue(radio, /obj/item/radio/proc/broadcast, list((message), src, TRUE), radio_delay)

/obj/item/radio/proc/broadcast(var/msg, var/mob/living/human/speaker, var/hardtohear = FALSE, var/needs_loc = TRUE)
...
/obj/item/radio/intercom/type96
	name = "Type 96"
	icon_state = "type96"
	item_state = "type96"
	frequency = JP_BASE_FREQ
	anchored = TRUE
	canhear_range = 1
	is_supply_radio = TRUE
	faction = JAPANESE

/obj/item/radio/intercom/type96/New()
	..()
	internal_channels = command_japan_channels.Copy()

/obj/item/radio/type94
	name = "Type 94"
	icon_state = "type94"
	item_state = "type94"
	frequency = JP_BASE_FREQ
...
/obj/item/radio/type94/command

/obj/item/radio/type94/New()
	..()
	internal_channels = default_japan_channels.Copy()

/obj/item/radio/type94/command/New()
	..()
	internal_channels = command_japan_channels.Copy()


/obj/item/radio/intercom/tby
	name = "TBY-8"
	icon_state = "tby8"
	item_state = "tby8"
	frequency = US_BASE_FREQ
	anchored = TRUE
	canhear_range = 1
	is_supply_radio = TRUE
	faction = AMERICAN

/obj/item/radio/intercom/tby/New()
	..()
	internal_channels = command_american_channels.Copy()
...
/obj/item/radio/scr/command

/obj/item/radio/scr/New()
	..()
	internal_channels = default_american_channels.Copy()

/obj/item/radio/scr/command/New()
	..()
	internal_channels = command_american_channels.Copy()
...
/obj/item/radio/Topic(href, href_list)
	...
	if (href_list["freq"])
		...
		switch (faction)
			...
			if (JAPANESE)
				// crash prevention + balance - Kachnov
				choices = processes.supply.japan_crate_types | processes.supply.japan_crate_types
				if (supplydrop_processing_objects_japan.len >= MAX_SUPPLYDROP_CRATES)
					usr << "<span class = 'danger'>There are too many items already arriving! Please wait before ordering more.</span>"
					return
			if (AMERICAN)
				// crash prevention + balance - Kachnov
				choices = processes.supply.american_crate_types | processes.supply.american_crate_types
				if (supplydrop_processing_objects_american.len >= MAX_SUPPLYDROP_CRATES)
					usr << "<span class = 'danger'>There are too many items already arriving! Please wait before ordering more.</span>"
					return
...
/obj/item/radio/proc/purchase(var/itemname, var/path, var/pointcost = FALSE)
	...
	if (ispath(path) && list(GERMAN, RUSSIAN, JAPANESE, AMERICAN).Find(faction))
...
/obj/item/radio/proc/setup_announcement_system(aname, channel)
	...
	mobs[aname] = new/mob/living/human/announcer
	var/mob/living/human/announcer/H = mobs[aname]
	H.name = aname
	H.real_name = aname
	...
	switch (faction)
		...
		if (JAPANESE)
			H.languages = list(new/datum/language/japanese)
		if (AMERICAN)
			H.languages = list(new/datum/language/english)
